...
function addExcelSheetHeaders(excelSheet: excel.Worksheet) {
  excelSheet.columns = [
    ...
    { key: "farmName", header: "Company", width: 30 },
    ...
    docs.docs.map(async (doc) => {
      try {
        ...
        if (sample["farmReference"] != undefined) {
          try {
            var farmReference = await (
              sample["farmReference"] as admin.firestore.DocumentReference
            ).get();
            var companyAdminUserInfo =
              farmReference.data()!["companyAdminUserInfo"];
            if (companyAdminUserInfo == undefined) {
              console.log("companyAdminUserInfo is undefined");
            } else {
              sample.companyAdminEmail = companyAdminUserInfo.email;
              sample.companyAdminFullName = companyAdminUserInfo.fullName;
            }
          } catch (e) {
...
export async function createSampleExcelSheetAndSendEmail(
  ...
  var bucket = storage.bucket("gs://agro-k-c5da2.appspot.com");
  ...
  console.log("error generating spreadsheet yesterdays samples", e);
