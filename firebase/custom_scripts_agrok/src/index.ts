...
async function runPrintFarmDocs(db: firestore.Firestore): Promise<void> {
...
async function getAdminInfoAndGlobalBarcodeCounts(
  db: firestore.Firestore
): Promise<void> {
  var adminInfoDoc = await firestore().collection("admin").doc("info").get();
  var adminInfoData = adminInfoDoc.data();
  if (adminInfoData == null) return;
  var assignedBarcodesAmount = adminInfoData["assignedBarcodesAmount"];
  var usedBarcodesAmount = adminInfoData["usedBarcodesAmount"];
  console.log(
    `assignedBarcodesAmount = ${assignedBarcodesAmount}, usedBarcodesAmount = ${usedBarcodesAmount}`
  );
  var assignedCount = 0;
  var usedCount = 0;
  var globalBarcodesDocs = (
    await firestore().collection("global_barcodes").get()
  ).docs;
  await Promise.all(
    globalBarcodesDocs.map(async (globalBarcodeDoc) => {
      var globalBarcodeData = globalBarcodeDoc.data();
      if (globalBarcodeData == null) return;
...
async function runPrintGlobalBarcodes(db: firestore.Firestore): Promise<void> {
  var globalBarcodesDocs = (
    await firestore().collection("global_barcodes").get()
  ).docs;
  var counter = 0;
  var barcodes: {
    barcode: string;
    farmReference: firestore.DocumentReference;
  }[] = [];
  await Promise.all(
    globalBarcodesDocs.map(async (globalBarcodeDoc) => {
      var globalBarcodeData = globalBarcodeDoc.data();
      ...
      if (farmReference != null && companyName == null) {
        console.log(
          `barcode = ${barcode}, farmReference = ${farmReference}, companyName = ${companyName}`
        );
        counter++;
...
async function runPrintGlobalBarcodeCount(
  db: firestore.Firestore
): Promise<void> {
  var globalBarcodesDocs = (
    await firestore().collection("global_barcodes").get()
...
async function runPrintSampleDoc(db: firestore.Firestore): Promise<void> {
  var sampleDoc = await firestore()
    .collection("samples")
...
async function runPrintSampleCount(db: firestore.Firestore): Promise<void> {
  var samplesDocs = (
    await firestore()
      .collection("samples")
  ...
  await Promise.all(
    samplesDocs.map(async (sampleDoc) => {
...
async function runMigrateFarmStates(db: firestore.Firestore): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
  await Promise.all(
    farmDocs.map(async (farmDoc) => {
      var farmData = farmDoc.data();
...
async function runFindGlobalBarcodes(db: firestore.Firestore): Promise<void> {
  var globalBarcodesDocs = (
    await firestore()
...
async function runCheckAdminBarcodes(db: firestore.Firestore): Promise<void> {
  var adminDoc = (
    await firestore().collection("admin").doc("info").get()
  ).data();
  ...
  await Promise.all(
    [...assignableBarcodes, ...scannableBarcodes].map(async (barcode) => {
      var globalBarcodeDoc = (
        await firestore()
          .collection("global_barcodes")
...
async function runMigrateCorrectBarcodeCounts(
  db: firestore.Firestore,
  ...
  await Promise.all(
    farmDocs.map(async (farmDoc) => {
      ...
      await Promise.all(
        globalBarcodesDocs.map(async (globalBarcodesDoc) => {
          var globalBarcodesData = globalBarcodesDoc.data();
          if (globalBarcodesData == null) return;
      ...
      console.log(
        `farmId = ${farmId} , OLD barcodesAssigned = ${farmData["barcodesAssigned"]} , usedBarcodesCount = ${farmData["usedBarcodesCount"]} // NEW barcodesAssigned = ${barcodesAssigned} , usedBarcodesCount = ${usedBarcodesCount}`
      );
      batch.update(firestore().collection("companies").doc(farmId), {
...
async function runBatchAddPrintedProperty(
  db: firestore.Firestore
  // batch: firestore.WriteBatch
): Promise<void> {
  var batch1 = db.batch();
  var samplesDocs = (await firestore().collection("samples").get()).docs;
...
async function runFindUsersWithEmail(db: firestore.Firestore): Promise<void> {
  var userDocs = (
    await firestore()
      .collection("users")
...
async function runFindUserChanges2(db: firestore.Firestore): Promise<void> {
  var userDocs = (
    await firestore()
      .collection("users")
      .where("firstName", "==", "JT")
...
async function runLogBarcodeCounts8(db: firestore.Firestore): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
  await Promise.all(
    farmDocs.map(async (farmDoc) => {
      var farmData = farmDoc.data();
      ...
      var globalBarcodesDocs = (
        await firestore()
          ...
          .where("farmReference", "==", farmReference)
          .get()
      ).docs.filter((doc) => doc.data()["sampleReference"] != null);
      console.log(
        `farmId = ${farmId} , usedBarcodeCounts = ${farmData["usedBarcodesCount"]} , globalBarcodesDocs length = ${globalBarcodesDocs.length}`
      );
    })
  );
}
...
async function runLogBarcodeCounts7(db: firestore.Firestore): Promise<void> {
  var globalBarcodesDocs = (
    await firestore()
      .collection("global_barcodes")
      .where("farmReference", "==", null)
  ...
  await Promise.all(
    globalBarcodesDocs.map(async (globalBarcodesDoc) => {
      ...
      console.log(
        `barcode = ${barcode} , samplesYoungDocs length = ${samplesYoungDocs.length}, samplesOldDocs length = ${samplesOldDocs.length}`
      );
    })
  );
}
...
async function runLogBarcodeCounts5(db: firestore.Firestore): Promise<void> {
  var samplesDocs = (await firestore().collection("samples").get()).docs;
  await Promise.all(
    samplesDocs.map(async (sampleDoc) => {
      var sampleData = sampleDoc.data();
      ...
      if (
        (youngSampleBarcode != null && youngGlobalBarcodesDoc == null) ||
        (oldSampleBarcode != null && oldGlobalBarcodesDoc == null)
      ) {
        console.log(
          `sampleId = ${sampleId} , youngSB = ${youngSampleBarcode} , oldSB = ${oldSampleBarcode} and youngGlobalBarcodesDoc = ${youngGlobalBarcodesDoc} and oldGlobalBarcodesDoc = ${oldGlobalBarcodesDoc}`
        );
...
async function runFindUserChanges(db: firestore.Firestore): Promise<void> {
  var userDoc = (
    await firestore()
      .collection("users")
  ...
  console.log(
    `samplesDoc length = ${samplesDoc.length} , userReference = ${userReference.path} , companyReference = ${companyReference.path}`
  );
}

//iterate through farms collection, for each doc, update "country" to "United States"
async function runBatchChangeFarmCountryToUSA(
  db: firestore.Firestore,
  batch: firestore.WriteBatch
): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
...
async function runLogBarcodeCounts6(db: firestore.Firestore): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
  await Promise.all(
    farmDocs.map(async (farmDoc) => {
      var farmData = farmDoc.data();
      ...
      console.log(
        `farm name = ${farmData["name"]} , farm id = ${farmData["id"]} , samplesCount = ${samplesDoc.length} , globalBarcodesCount = ${globalBarcodesDoc.length}`
      );
    })
  );
}
...
async function runSeeSampleChanges(db: firestore.Firestore): Promise<void> {
  var samplesDocs = (await firestore().collection("samples").get()).docs;
  await Promise.all(
    samplesDocs.map(async (sampleDoc) => {
      var sampleData = sampleDoc.data();
      if (sampleData == null) return;
      var sampleId = sampleData["id"];
      var changes = sampleData["changes"] ?? [];
      console.log(
        `changes length = ${changes?.length} , sampleId = ${sampleId}`
      );
    })
  );
}
...
async function runLogBarcodeCounts4(db: firestore.Firestore): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
  await Promise.all(
    farmDocs.map(async (farmDoc) => {
      console.log(
        "---- starting with farm name = " + farmDoc.data()["name"] + " ----"
      );
      var farmData = farmDoc.data();
      if (farmData == null) return;
      var farmReference = farmDoc.ref;
      ...
      samplesDoc.forEach((sampleData) => {
        if (sampleData == null) return;
        if (
          sampleData["youngSampleBarcode"] != null &&
          sampleData["youngSampleBarcode"].length > 0
        ) {
          sampleBarcodeCount++;
          if (
            globalBarcodesDoc.find((doc) => {
        ...
        if (
          sampleData["oldSampleBarcode"] != null &&
      ...
      console.log(
        `farm name = ${farmName} , farm id = ${farmId} , samplesCount = ${sampleBarcodeCount} , globalBarcodesCount = ${globalBarcodesCount}`
      );
      console.log(
        "---- end with farm name = " + farmDoc.data()["name"] + " ----"
      );
    })
  );
}
...
async function runLogBarcodeCounts3(db: firestore.Firestore): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
  await Promise.all(
    farmDocs.map(async (farmDoc) => {
      var farmData = farmDoc.data();
      ...
      var globalBarcodesSampleReferenceNullCount = globalBarcodesDoc.filter(
        (globalBarcodesDoc) =>
          globalBarcodesDoc.data()["sampleReference"] == null
      ).length;
      var globalBarcodesSampleReferenceNotNullCount = globalBarcodesDoc.filter(
        (globalBarcodesDoc) =>
          globalBarcodesDoc.data()["sampleReference"] != null
      ).length;
      console.log(
        `${farmUsedCount == globalBarcodesSampleReferenceNotNullCount} , ${
          farmBarcodesAssigned - farmUsedCount ==
          globalBarcodesSampleReferenceNullCount
        } , farmUsedCount = ${farmUsedCount} , farmBarcodesAssigned = ${farmBarcodesAssigned} , globalBarcodesSampleReferenceNullCount = ${globalBarcodesSampleReferenceNullCount} , globalBarcodesSampleReferenceNotNullCount = ${globalBarcodesSampleReferenceNotNullCount}`
      );
    })
  );
}
...
async function runLogBarcodeCounts2(db: firestore.Firestore): Promise<void> {
  var infoDoc = await firestore().collection("admin").doc("info").get();
  ...
  await Promise.all(
    farmDocs.map(async (farmDoc) => {
      ...
      console.log(
        `farm name = ${farmData["name"]} , farm id = ${farmData["id"]} , farmUsedCount = ${farmUsedCount}`
      );
    })
  );
...
async function runCompanyAdminUserInfoFromFarm(
  db: firestore.Firestore
): Promise<void> {
  var farmDoc = await firestore()
    .collection("companies")
...
async function runBatchChangeFarmAdmin(
  db: firestore.Firestore,
  ...
  if (farmData == null) return;
  var farmReference = farmDoc.ref;
  var userDoc1 = await firestore()
    .collection("users")
    .where("email", "==", "jtmcclellan+costagroup@gmail.com")
    .get();
  var userDoc2 = await firestore()
    .collection("users")
  ...
  if (userDoc1.docs.length == 1 && userDoc2.docs.length == 1) {
    ...
    console.log(
      `user1RefId = " + ${userDoc1Reference.id} , user2RefId = " + ${userDoc2Reference.id}`
    );
    var farmReferences1 = userDoc1Data["farmReferences"] as [];
    var farmReferences2 = userDoc2Data["farmReferences"] as [];
    if (farmReferences1 != null && farmReferences2 != null) {
      console.log("1");
      var indexToModify1 = farmReferences1.findIndex(
        (farmReference1) =>
          (farmReference1["farmReference"] as any).id == farmReference.id
      );
      var indexToModify2 = farmReferences2.findIndex(
        (farmReference2) =>
          (farmReference2["farmReference"] as any).id == farmReference.id
      );
      console.log(
        `indexToModify1 = ${indexToModify1} // indexToModify2 = ${indexToModify2}`
      );
      if (indexToModify1 != -1 && indexToModify2 != -1) {
        console.log("2");
        (farmReferences1[indexToModify1] as any)["isAdmin"] = false;
        (farmReferences2[indexToModify2] as any)["isAdmin"] = true;
...
async function runCheckFarmName(db: firestore.Firestore): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
  await Promise.all(
    farmDocs.map(async (farmDoc) => {
      var farmData = farmDoc.data();
...
async function runCheckFarmReference(db: firestore.Firestore): Promise<void> {
  var sampleDocs = (await firestore().collection("samples").get()).docs;
  await Promise.all(
    sampleDocs.map(async (sampleDoc) => {
      var sampleData = sampleDoc.data();
      var farmReference = sampleData["farmReference"];
      if (farmReference != null) {
        var farmReferenceId = (farmReference as any).id;
...
async function runCheckEnabledUndefined(
  db: firestore.Firestore
): Promise<void> {
...
async function runBatchMigrateCompanyAdminUserInfo2(
  db: firestore.Firestore,
  batch: admin.firestore.WriteBatch
): Promise<void> {
  // var farmDocs = (await firestore().collection("companies").get()).docs;
  ...
  await Promise.all(
    userDocs.map(async (userDoc) => {
      ...
      if (farmReferences != null) {
        await Promise.all(
          farmReferences.map(async (farmReference: any) => {
            var farmReferenceId = (farmReference["farmReference"] as any).id;
            var farmReferenceDoc = await firestore()
              .collection("companies")
              .doc(farmReferenceId)
              .get();
...
async function runBatchMigrateGlobalBarcodes(
  db: firestore.Firestore
): Promise<void> {
  var globalBarcodeDocs = (
    await firestore().collection("global_barcodes").get()
  ).docs;
  var barcodesAssigned = 0;
  var barcodesUsed = 0;
  await Promise.all(
    globalBarcodeDocs.map(async (globalBarcodeDoc) => {
      var globalBarcodeData = globalBarcodeDoc.data();
      var farmReference = globalBarcodeData["farmReference"];
      var sampleReference = globalBarcodeData["sampleReference"];
...
async function runBatchMigrateAddEnabledFieldToUser(
  db: firestore.Firestore,
  batch: admin.firestore.WriteBatch
): Promise<void> {
  var userDocs = (await firestore().collection("users").get()).docs;
...
async function updateAdminInfoBarcodeCounts(
  db: firestore.Firestore,
  batch: admin.firestore.WriteBatch
): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
  ...
  if (data != null) {
    console.log(
      `totalBarcodesAssigned = ${totalBarcodesAssigned} / totalBarcodesUsed = ${totalBarcodesUsed} / data.assignedBarcodesAmount = ${data["assignedBarcodesAmount"]} / data.usedBarcodesAmount = ${data["usedBarcodesAmount"]}`
    );
    var body = new Map<String, any>();
...
async function runLogBarcodeCounts(db: firestore.Firestore): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
  await Promise.all(
    farmDocs.map(async (farmDoc) => {
      var farmData = farmDoc.data();
      ...
      console.log(
        `farmName = ${farmName} / usedBarcodesCount = ${usedBarcodesCount} / farmBarcodeCount = ${farmBarcodeCount}`
      );
      //update these values
      // var body = new Map<String, any>();
...
async function updateFarmBarcodeCounts(
  db: firestore.Firestore,
  batch: admin.firestore.WriteBatch
): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
...
async function deleteSamplesXXX(
  db: firestore.Firestore,
  batch: admin.firestore.WriteBatch
): Promise<void> {
  var sampleDocs = (await firestore().collection("samples").get()).docs;
...
async function setUsedBarcodeCount(
  db: firestore.Firestore,
  batch: admin.firestore.WriteBatch
): Promise<void> {
  var farmDocs = (await firestore().collection("companies").get()).docs;
...
async function runBatchMigrateCompanyAdminUserInfo(
  db: firestore.Firestore,
  batch: admin.firestore.WriteBatch
): Promise<void> {
  ...
  await Promise.all(
    userDocs.docs.map(async (userDoc) => {
      ...
      var farmReferences = user["farmReferences"] as any[];
      if (farmReferences != null) {
        await Promise.all(
          farmReferences
            ...
            .map(async (userFarmCombo) => {
              ...
              if (farmDoc.exists) {
                ...
                console.log(
                  `let's set user ${user.email} as admin of farm ${
                    farmDoc.data()["name"]
                  }`
                );
                batch.update(farmDoc.ref, {
