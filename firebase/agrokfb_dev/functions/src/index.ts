...
exports.updateCompanyName = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    //update users doc, where farmReferences array of objects has a an item where farmReference.id == companyId
    const companyUsers = (
...
//cloud function that listens to create in farms collection, checks if users list if not empty, then take first reference, get document from "users" collection, and set companyAdminUserInfo as { fullName: firstName + lastName, email: email, userReference: userReference }
...
//cloud function when doc is created in "users" collection, just add "enabled" property set to true
...
exports.updateAdminInfo = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const oldBarcodesAssigned = beforeData.barcodesAssigned;
    const newBarcodesAssigned = afterData.barcodesAssigned;
    const oldBarcodesPurchased = beforeData.barcodesPurchased;
    const newBarcodesPurchased = afterData.barcodesPurchased;
    const oldUsedBarcodesCount = beforeData.usedBarcodesCount;
    const newUsedBarcodesCount = afterData.usedBarcodesCount;
    if (
      !(
    ...
    //rewrite the code i deleted here
  });

exports.notifyUsersSampleStatusChanged = functions.firestore
  .document("samples/{sampleId}")
  .onUpdate(async (change, context) => {
    var sampleStatusLabStarted = "Lab Started";
    var sampleStatusLabCompleted = "Lab Completed";
    var sampleStatusLabCancelled = "Lab Cancelled";
    var beforeData = change.before.data();
    var afterData = change.after.data();
    var oldSampleStatus = beforeData.status;
    var newSampleStatus = afterData.status;
    if (
      oldSampleStatus == newSampleStatus ||
      (newSampleStatus != sampleStatusLabStarted &&
        newSampleStatus != sampleStatusLabCancelled &&
        newSampleStatus != sampleStatusLabCompleted)
    )
      return;
    if (oldSampleStatus == newSampleStatus) return;
    var sampleId = context.params.sampleId;
    var companyReference = afterData.companyReference;
    if (companyReference == undefined) return;
    var companyDoc = await companyReference.get();
    var companyData = companyDoc.data();
    if (companyData == undefined) return;
    var usersToNotifyPushNotification = Array<firestore.DocumentReference>();
    if (companyData.companyAdminUserInfo?.userReference != undefined) {
      usersToNotifyPushNotification.push(
        companyData.companyAdminUserInfo
          .userReference as firestore.DocumentReference
      );
    }
    if (
      afterData.userReference != undefined &&
      !usersToNotifyPushNotification.some(
        (existingItem) => existingItem.id == afterData.userReference.id
      )
    ) {
      usersToNotifyPushNotification.push(
        afterData.userReference as firestore.DocumentReference
      );
    }
    if (
      afterData.assignedTo != undefined &&
      !usersToNotifyPushNotification.some(
        (existingItem) => existingItem.id == afterData.assignedTo.id
      )
    ) {
      usersToNotifyPushNotification.push(
        afterData.assignedTo as firestore.DocumentReference
      );
    }
    await Promise.all(
      usersToNotifyPushNotification.map(
        async (userReference: firestore.DocumentReference) => {
          var userDoc = await userReference.get();
          var userData = userDoc.data();
          if (userData == undefined) return;
          var notificationSettings = userData.notificationSettings;
          if (
            newSampleStatus == sampleStatusLabStarted &&
            notificationSettings.sampleStarted
          ) {
            var title: string = `Sample Started for ${afterData.crop} ${afterData.field}`;
            var body: string = "We have started analysis on your sample.";
            ...
            await sendNotification(title, body, userData.deviceTokens);
          }
          if (
            newSampleStatus == sampleStatusLabCancelled &&
            notificationSettings.sampleCancelled
          ) {
            var title: string = `Sample Cancelled for ${afterData.crop} ${afterData.field}`;
            var body: string =
              "There was an issue with your sample, so we have cancelled the analysis.";
            await sendNotification(title, body, userData.deviceTokens);
            await userReference.update({
              notifications: firestore.FieldValue.arrayUnion({
                title: title,
          ...
          if (
            newSampleStatus == sampleStatusLabCompleted &&
            notificationSettings.sampleComplete
          ) {
            var titlePushNotification: string = `Sample Completed for ${afterData.crop} ${afterData.field}`;
            var bodyPushNotification: string =
              "We have finished analysis on your sample. You will be receiving the results shortly.";
            ...
            await userReference.update({
              notifications: firestore.FieldValue.arrayUnion({
                title: titlePushNotification,
                description: bodyPushNotification,
                createdAt: firestore.Timestamp.now(),
              }),
            });
          }
          if (newSampleStatus == sampleStatusLabCompleted) {
            var grower = afterData.grower;
            var growerNotificationDoc = await firestore()
              .collection("grower_notifications")
              .doc(grower)
              .get();
            var growerNotificationData = growerNotificationDoc.data();
            if (growerNotificationData == undefined) return;
            var userEmailsToNotify = growerNotificationData.userEmails;
            console.log(`userEmails = ${userEmailsToNotify}`);
            var subjectEmail = `Sample Completed for ${afterData.grower}`;
            var bodyEmail = `We have finished analysis on your sample. You will be receiving the results shortly.`;
            await sendEmail(subjectEmail, bodyEmail, userEmailsToNotify);
          }
        }
      )
    );
...
function sendEmail(subject: string, message: string, emails: string[]) {
  return firestore()
    .collection("mail")
    .doc()
    .set({
...
function sendNotification(title: string, body: string, tokens: string[]) {
...
export const sendYesterdaySamples = functions.pubsub
  .schedule("0 0 * * *")
  .onRun(async (context) => {
...
export const deleteExpiredReservedBarcodes = functions.pubsub
  .schedule("0 * * * *")
  .onRun(async (context) => {
    var currentDate = Date.now();
    var expiredReservedBarcodes = (
      await firestore()
        .collection("global_barcodes")
        .where("reservedTimestamp", "<", currentDate - 3600000)
        .get()
    ).docs;
    await Promise.all(
      expiredReservedBarcodes.map(async (barcodeDoc) => {
        await barcodeDoc.ref.update({
          reservedTimestamp: firestore.FieldValue.delete(),
          reservedBy: firestore.FieldValue.delete(),
        });
      })
    );
  });
...
export const getConvergePayToken = functions.https.onRequest(
  async (req, res) => {
    cors(req, res, () => {
      if (req.method === "POST") {
        const url =
...
export const assignBarcodesFromPurchase = functions.https.onRequest(
  async (req, res) => {
    cors(req, res, async () => {
      if (req.method === "POST") {
        try {
